...
const filterKeys = ["keyword", "jobType", "experienceLevel", "location", "salaryMin", "salaryMax", "currency", "skills", "sort"] as const;

export function JobSearch({ showSortByMatch = false }: { showSortByMatch?: boolean } = {}) {
  ...
  const activeFilterCount = filterKeys.filter(
    (k) => k !== "keyword" && k !== "sort" && searchParams.get(k)
  ).length;
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {showSortByMatch && (
          <select
            ...
            className={selectClass}
          >
            <option value="">{t("sortByDate")}</option>
        ...
        >
          ...
          {activeFilterCount > 2 && (
            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-primary text-[10px] font-bold text-primary-foreground">
              {activeFilterCount - (searchParams.get("jobType") ? 1 : 0) - (searchParams.get("experienceLevel") ? 1 : 0)}
            </span>
          )}
          <ChevronDown
      ...
      {activeFilterCount > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {searchParams.get("jobType") && (
            <FilterTag
              label={t(searchParams.get("jobType")!)}
              onRemove={() => updateFilter("jobType", "")}
            />
          )}
          {searchParams.get("experienceLevel") && (
            <FilterTag
              label={t(searchParams.get("experienceLevel")!)}
              onRemove={() => updateFilter("experienceLevel", "")}
            />
          )}
          {searchParams.get("location") && (
            <FilterTag
          ...
          {searchParams.get("currency") && (
            <FilterTag
              label={searchParams.get("currency")!}
              onRemove={() => updateFilter("currency", "")}
            />
          )}
          {searchParams.get("salaryMin") && (
            <FilterTag
              label={`${t("salaryMin")}: ${Number(searchParams.get("salaryMin")).toLocaleString()}`}
              onRemove={() => updateFilter("salaryMin", "")}
            />
          )}
          {searchParams.get("salaryMax") && (
            <FilterTag
              label={`${t("salaryMax")}: ${Number(searchParams.get("salaryMax")).toLocaleString()}`}
              onRemove={() => updateFilter("salaryMax", "")}
            />
          )}
          {activeSkills.map((skill) => (
            <FilterTag
